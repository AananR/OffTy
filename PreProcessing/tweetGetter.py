    ...
    with open(idfilepath, 'rb') as idfile:
        for line in idfile:
            ...
            try:
                ...
                masterDict[tweet_id] = tweet.full_text.encode('UTF-8')
...
def get_tweet_list(twapi, idlist):
    ...
    for tweet in tweets:
        masterDict[tweet.id] = tweet.full_text
...
def get_tweets_bulk(twapi, idfilepath):
    ...
    ##################
    reader = csv.DictReader(open(idfilepath))
    ...
    newName = "new_hate_Two"
    js.writeToJSONFile(".",newName, masterDict)
