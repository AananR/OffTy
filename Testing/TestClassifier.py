...
def classify(tweets):
    ...
    dfNormal = df[df["label"] == "normal"].sample(n=14000)
    ...
    df2 = pd.read_csv("searchedIDS/" + tweets)
    ...
    # Predict the results
    test_predicted_svm = clf_svm.predict(tweets_counts)
...
#print(classify("OHonours_tweets.csv"))
...
def classifyAccuracy(tweets):
    ...
    df2 = pd.read_csv("Testing/" + tweets)
    ...
    #############################################################################################################################################################################################
    # Training the model with an SVM using a linear kernel (less computationally intensive)
    clf_svm = svm.SVC(kernel="linear", max_iter=6000).fit(train_counts, train_tags)
    ...
    for doc, category in zip(test_tweets,
                             test_predicted_svm):  # zip allows to go through two lists simultaneously
        print('%r => %s\n' % (doc, category))
...
classifyAccuracy("OHonours_Testing_041120.csv")